...
static IObjPtr (Func_copy0)(IObjPtr src1, IObjPtr dst1){
     Utf8String* src = (Utf8String*)src1;
     Utf8String* dst = (Utf8String*)dst1;
     if(dst == NULL){
         dst = (Utf8String*)ALLOC(sizeof(Utf8String));
         __Utf8String_init(dst);
     }else{
        FREE(dst->data);
     }
     dst->data = ALLOC(src->_alloced);
     dst->_alloced = src->_alloced;
...
static int (Func_equals0)(IObjPtr src1, IObjPtr dst1){
    ...
    if(memcmp(src->data, dst->data, src->length) != 0){
        return kState_FAILED;
    }
    return kState_OK;
}
static uint32 (Func_hash0)(IObjPtr src1, uint32 seed){
    Utf8String* src = (Utf8String*)src1;
    return fasthash32(src->data, src->length, seed);
}
static void (Func_dump0)(IObjPtr src1, hstring* hs){
    Utf8String* src = (Utf8String*)src1;
    hstring_append2(hs, (const char*)src->data, src->length);
}
...
Utf8String *utf8_create() {
    ...
    uni_str->data = ALLOC(length * sizeof(utf8_char));
...
int utf8_indexof_pos(Utf8String *a1, Utf8String *a2, int a1_pos) {
    ...
    if (a1_pos >= (int)a1->length || a1_pos < 0)return -1;
...
int utf8_last_indexof_pos(Utf8String *a1, Utf8String *a2, int a1_rightpos) {
    if (a1 == NULL || a2 == NULL)return -1;
    if (a1_rightpos >= (int)a1->length || a1_rightpos < 0)return -1;
...
void utf8_replace(Utf8String *a1, Utf8String *a2, Utf8String *a3) {
    ...
    utf8_cstr(a2);
    utf8_cstr(a3);
    _utf8_replace_impl(a1, (char *) a2->data, a2->length,
                       (char *) a3->data, a3->length);
}
...
static inline int _utf8_equals_impl(utf8_char *a1,
                                    int count1, utf8_char *a2, int count2) {
    if (a1 == NULL && a2 == NULL)return 1;
...
int _utf8_space_require(Utf8String *ustr, int need) {
    ...
    data = REALLOC2(ustr->data, sizeof(utf8_char) * newsize);
...
int utf8_insert(Utf8String *ustr, int index, utf8_char data) {
    ...
    if (index < 0 || index > (int)ustr->length) {
        return 0;
    }
...
void utf8_remove_range(Utf8String *ustr, int index, int length) {
    ...
    if (index < 0 || length < 0 || index + length > (int)ustr->length) {
        return;
    }
...
uint32 _utf8_hashCode(Utf8String *ustr){
    if(ustr->length == 0){
        return DEFAULT_HASH_SEED;
    }
    return fasthash32(ustr->data, ustr->length, DEFAULT_HASH_SEED);
}
