...
#define array_map_n_prepare_keySize(ptr, size)\
    if(ptr->alloc_key_size < size){\
        ptr->keys = H7_REALLOC(ptr->keys, ptr->alloc_key_size, HMAX(GROWUP_HALF(ptr->alloc_key_size), size));\
        ptr->alloc_key_size = size;\
    }

#define array_map_n_prepare_valueSize(ptr, size)\
    if(ptr->alloc_val_size < size){\
        ptr->values = H7_REALLOC(ptr->values, ptr->alloc_val_size, HMAX(GROWUP_HALF(ptr->alloc_val_size), size));\
...
#define array_map_n_prepare_entryCount(ptr, count)\
    if(ptr->hash_count < count){\
        ...
        ptr->key_hashes = (uint32*)H7_REALLOC(ptr->key_hashes, sizeof (uint32) * ptr->hash_count, sizeof (uint32) * c);\
        ptr->key_lens = (uint16*)H7_REALLOC(ptr->key_lens, sizeof (uint16) * ptr->hash_count, sizeof (uint16) * c);\
        ptr->val_lens = (uint16*)H7_REALLOC(ptr->val_lens, sizeof (uint16) * ptr->hash_count, sizeof (uint16) * c);\
        ptr->hash_count = c;\
    }
