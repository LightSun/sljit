...
void SLJITHelper::emitLoadArray(SPSentence st){
    ...
    auto ls_obj = ip.extra->objIdxes[0];
    auto ls_data = ip.extra->objIdxes[1];
    auto ls_shape = ip.extra->objIdxes[2];
    auto ls_curOffset = ip.extra->objIdxes[3]; //runtime in bytes
    auto ls_subEleSize = ip.extra->objIdxes[4];
    //load ptr, offset, real_data addr
    //SP for LS
    if(ip.isLS()){
    ...
    sljit_emit_op1(C, SLJIT_MOV, reg_p2, 0, SLJIT_IMM, 0);
    //
    //load datas
    ...
    sljit_emit_op1(C, SLJIT_MOV, LS_R, LS_OFFSET(ls_subEleSize), SLJIT_R0, 0);
    //cur offset
    sljit_emit_op1(C, SLJIT_MOV, LS_R, LS_OFFSET(ls_curOffset), SLJIT_IMM, 0);
...
void SLJITHelper::emitLoadArrayElement(SPSentence st){
    ...
    auto& ele_op = st->ip;
    auto& arr_op = st->left;
    auto arrIds = arr_op.indexAsArrayIds();
    //ext index as field index.

    int rs_key = RS->save();
    // load data and eleSize
    int reg_data = RS->nextReg(false);
    sljit_emit_op1(C, SLJIT_MOV, reg_data, 0, LS_R, LS_OFFSET(arrIds.data));
    //---------------------------
    //compute real-offset
    int reg_f = RS->nextReg(false);
